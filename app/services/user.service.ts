...
export const userService = {
  ...
  getAll: (params?: Record<string, any>) => api.getAll("/user", params ?? {}, { withAuth: true }),
  ...
  delete: (id: string) => api.delete(`/user/${id}`, { withAuth: true }),
  assignedToCompany: (data: any) => api.post("/user/assign-company", data, { withAuth: true }),
  unassignCompany: (data: any) => api.post("/user/unassign-company", data, { withAuth: true }),
  ...
  upload: (id: string, data: any) => api.post(`/user/upload/${id}`, data, { withAuth: true }),
};
